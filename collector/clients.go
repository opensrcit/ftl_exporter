...
func (c *clientCollector) update(client *ftl_client.FTLClient, ch chan<- prometheus.Metric) error {
	...
	for _, hits := range clients.List {
		ch <- prometheus.MustNewConstMetric(c.topClientsToday, prometheus.GaugeValue, float64(hits.Count.Value), hits.Domain)
	}
	...
	for _, hits := range blockedClients.List {
		ch <- prometheus.MustNewConstMetric(c.topBlockedClientsToday, prometheus.GaugeValue, float64(hits.Count.Value), hits.Domain)
	}
